...
Notes:
* `app` is an abbreviation for EditContextIssue, use the acronym of your choice and replace app with that (for example abc) using exact match - case sensitive find and replace in this file and AdStack.cs.
...
pulumi config set EditContextIssue.Iac:sql-server-app-db-admin-id
pulumi config set EditContextIssue.Iac:sql-server-app-db-admin-password --secret

# Provide SMTP server's host, port, user, pass and default email sender.
pulumi config set EditContextIssue.Iac:default-email-from
pulumi config set EditContextIssue.Iac:email-server-host
pulumi config set EditContextIssue.Iac:email-server-port
pulumi config set EditContextIssue.Iac:email-server-userName
pulumi config set EditContextIssue.Iac:email-server-password --secret

# Create and provide an identity certificate password
pulumi config set EditContextIssue.Iac:identity-certificate-password --secret
```
